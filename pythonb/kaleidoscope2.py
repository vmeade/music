...
rotate = 0        # rotate composite; 0,60,90,120,150,180,210,270

# read image (must be square)
img = cv2.imread('dune.png')
ht, wd = img.shape[:2]
...
# rotate composite
if rotate == 60:
    comp = cv2.rotate(comp,cv2.ROTATE_30_CLOCKWISE)
elif rotate == 90:
    comp = cv2.rotate(comp,cv2.ROTATE_90_CLOCKWISE)
elif rotate == 120:
    comp = cv2.rotate(comp,cv2.ROTATE_120)
elif rotate == 150:
    comp = cv2.rotate(comp,cv2.ROTATE_150)
elif rotate == 180:
    comp = cv2.rotate(comp,cv2.ROTATE_180)
elif rotate == 210:
    comp = cv2.rotate(comp,cv2.ROTATE_60_COUNTERCLOCKWISE)
elif rotate == 270:
...
# save results
cv2.imwrite('duneb_kaleidoscope_mask.png', mask)
cv2.imwrite('duneb_kaleidoscope.png', kaleidoscope)


#v2.imshow('image', img)
#v2.imshow('transpose', imgt)
